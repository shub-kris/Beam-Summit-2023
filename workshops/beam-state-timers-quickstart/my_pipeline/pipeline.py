...
class FindSessions(beam.DoFn):
    ...
    def process(self, element: tuple[str, TaxiPoint],
        ...
        if point.ride_status == "dropoff":
            ...
            # if not, keep waiting
            max_ts = Timestamp(max_timestamp_seen.read()).seconds()
            expiration_second: int = max_ts + 8 * 60
            gc_timer.set(Timestamp(seconds=expiration_second))
    ...
    def expiry_callback(
        ...
        # accumulated so far
        session = self._calculate_session(key_state.read(),
                                          taxi_ride_events_bag,
