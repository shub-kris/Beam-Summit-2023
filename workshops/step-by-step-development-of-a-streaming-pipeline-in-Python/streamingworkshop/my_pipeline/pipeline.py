...
class AddKeysToTaxiRides(beam.PTransform):
    ...
    def expand(self, pcoll):
        # identify unique keys per Taxipoint
        # TODO - complete DoFn
        keys: PCollection[tuple(str, TaxiPoint)] = pcoll | "addkeys" >> beam.WithKeys(lambda x: x.ride_id)
        # keys: PCollection[tuple(str, TaxiPoint)] = pcoll | "addkeys" >> beam.Map(lambda x: (x.ride_id, x))
        return keys
...
class TaxiSessioning(beam.PTransform):
    ...
    def expand(self, pcoll):
        ...
        windowed: PCollection[tuple(str, TaxiPoint)] = pcoll | "sessioning" >> beam.WindowInto(
            windowfn=Sessions(480.0), #8mins --> Where in event time to calculate ?
            trigger=AfterWatermark(), #when in processing to emit results
            accumulation_mode=AccumulationMode.DISCARDING,  # how to do refinement of results relate?
